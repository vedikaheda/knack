---
name: generate_brd_from_attendee_transcript
description: Use this skill when an Attendee bot has finished transcribing a meeting and Knack should turn the transcript into a BRD in Proof.
---

# Generate BRD From Attendee Transcript

Use this skill when:

- an internal hook says an Attendee transcript is ready
- the message includes an Attendee `bot_id`
- the user asks Knack to create a BRD from a completed Attendee meeting

## Workflow

1. Extract the Attendee `bot_id`.
2. Call `get_attendee_transcript`.
...
- Always fetch the transcript with `get_attendee_transcript`.
...
- Do not expose Attendee API credentials, internal hook details, or Proof owner secrets.
...
- mention important missing information only if the transcript was materially incomplete
